...
class AuditTrail:
    ...
    def __init__(self, secret_key: Optional[str] = None):
        ...
        if not resolved_key:
            ...
            import warnings
            warnings.warn(
                "AUDIT_SECRET_KEY not set. Generated ephemeral key — audit trail integrity "
                "will be lost on restart. Set AUDIT_SECRET_KEY in production.",
                RuntimeWarning,
                stacklevel=2,
            )
    ...
    def verify_integrity(self) -> bool:
        for i, entry in enumerate(self.logs):
            ...
            if entry["prev_hash"] != prev:
                return False
            # Verify HMAC signature to detect any field tampering
